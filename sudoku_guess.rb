...
class Sudoku
  ...
  def solve(test_board = @board.clone)
    ...
    puts test_board if test_board.solved?
    abort("We win!") if test_board.solved?
    x = 0
    ...
    possibles.each do |test_case|
      ...
      return test_board_clone if solve(test_board_clone)
      end
    return test_board_clone if test_board.solved?
  end
 
  
  def possible_values(x_coord, y_coord, test_board)
    return nil if test_board.solved?
    possibles =[]
    (1..9).each do |number|
